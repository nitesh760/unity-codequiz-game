...
public class medium : MonoBehaviour
{
    ...
    {
        // Create a new score entry with correct formatting
        string newScoreEntry = " Medium Score: " + newScore + " | Date: " + System.DateTime.Now.ToString("dd-MM-yyyy HH:mm:ss");
        ...
        {
            scoreList.RemoveAt(0); // Remove the oldest score
        }

        // Save back to PlayerPrefs with line breaks for proper display
        PlayerPrefs.SetString("SavedScores", string.Join("\n", scoreList));
        PlayerPrefs.Save();
